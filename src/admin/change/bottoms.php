...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['bottoms']) && $_FILES['bottoms']['error'] === UPLOAD_ERR_OK) {
        $targetDirectory = "../uploads/";
        $bottoms = "pic_" . date('YmdHis') . "_" . rand(10, 999999) . "_" . $_FILES["bottoms"]["name"];
        $targetFilePath = $targetDirectory . $bottoms;

        // Check if the file has been uploaded and move it to the target directory
        if (move_uploaded_file($_FILES['bottoms']['tmp_name'], $targetFilePath)) {
            // Prepare SQL query to insert image name into the database
            $sql = "INSERT INTO images (bottoms) VALUES (?)";
            $stmt = $conn->prepare($sql);

            // Bind the image name as parameter and execute the query
            $stmt->bind_param("s", $bottoms);
            if ($stmt->execute()) {
