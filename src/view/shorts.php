...
.product-details {
  ...
  <!-- Page Title -->
  <title>SHORTS - SLT Clothing</title>
</head>
<body>
     <?php include 'nav.php';?> 
                    ...
                    <div>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                              ...
                              <li class="breadcrumb-item active"><a href="../view/shorts.php">shirts</a></li>
                            </ol>
                        </nav>       
                    </div>
                </div>
        ...
        <div class="d-flex justify-content-between items-center pt-5 pb-4 flex-column flex-lg-row">
            <div>
                <nav aria-label="breadcrumb"></nav>
                               <?php
$sql = "SELECT COUNT(*) as shorts FROM products WHERE category = 'shorts'";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
$shorts = $row['shorts'];

echo '<p class="m-0 small"><span class="text-muted">Showing</span> - ' . $shorts . ' items</p>';
?>
...
$sql = "SELECT * FROM products WHERE category = '$shorts' ORDER BY PID DESC LIMIT $items_per_page OFFSET $offset";
$result = $conn->query($sql);
...
?>
     <div class="col-12 text-center py-3">
        <div class="alert alert-info" role="alert">
            <h4 class="alert-heading">No Shorts available</h4>
...
<script>
    function applyFilters() {
        ...
        fetch('sortssearch.php?search=' + filterSearch.value.trim())
